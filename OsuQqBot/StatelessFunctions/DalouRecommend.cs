    ...
    class DalouRecommend : IStatelessFunction
    {
        ...
        private bool DalouMessage(EndPoint endPoint, string message)
        {
            ...
            if (messages.Length <= 3)
                return false;
            if (messages[0] != "[CQ:at,qq=122866607] ")
                return false;
            if (messages[1] != "bleatingsheep的推荐图如下:")
                return false;
            if (messages[2] != "Bid, Mod, 参考pp, 参考acc")
                return false;
            ...
            for (int i = 3; i < messages.Length; i++)
            {
                ...
                if (!match.Success || !int.TryParse(match.Groups[1].Value, out int bid))
                {
                    ...
                    if (isPrivate)
                        qq.SendGroupMessageAsync(DebugGroup, "失败，" + (i + 1));
                    return false;
                }
                string mods = match.Groups[2].Value;
                bool hasHT = false;
                bool hasDT = false;
                if (mods.Contains("DT") || mods.Contains("HT"))
                    hasDT = true;
            ...
            var t = Task.Run(() =>
            {
                ...
                using (var stringWriter = new System.IO.StringWriter())
                {
                    ...
                    foreach (var beatmap in beatmaps)
                    {
                        ...
                        if (beatmap.HitLength < 170 ||
                            ps.Any(b => b.hasDT && b.bid == beatmap.Id) && beatmap.HitLength < 225
                            )
                        {
                            qq.SendMessageAsync(endPoint, "!banmap " + beatmap.Id);
                            if (isPrivate)
                                qq.SendGroupMessageAsync(DebugGroup, "!banmap " + beatmap.Id);
                        }
                    ...
                    if (isPrivate)
                        qq.SendGroupMessageAsync(DebugGroup, m, true);
                }
            });
            Task.Run(() =>
            {
